...
- [ ] **シナリオテストの導入 (doc/test/scenario)**:
    - [ ] 実機から抽出したサンプルデータを用い、「画面遷移を通じたデータの整合性」を検証するテストを定義・実装。
- [ ] **インクリメンタル・タイピング・テストの標準化**:
    - [ ] 1文字入力ごとにアサーションを行う UI テストを主要な入力画面に追加し、自動補完やリセットループを機械的に検出。
- [ ] **ガード節の可視化 (AuditLog)**:
    - [ ] 基盤クラス（`PersonBaseUiStateViewModel`）のガード処理において、スキップが発生した事実をログに残す仕組みを導入。

## 5. ViewModel の堅牢化と品質担保（旧 ViewModel 改善計画より）
...
## 🎯 5. ViewModel の堅牢化 ロードマップ

「5. ViewModel の堅牢化と品質担保」の詳細な実施ステップです。

### 5.1. BatchInputViewModel の堅牢化
- [ ] **保存処理の異常系テスト**:
    - [ ] `insertHealthDataBatch` 実行中に例外をスローし、AuditLog に `ERROR` が正しく記録されることを検証。
    - [ ] 保存失敗後も `UiState` の入力値（身長・体重等）がクリアされずに保持されていることを検証。
- [x] **ロジックの整理（将来の拡張性確保）**:
    - [x] カテゴリごとの `when` 分岐を整理し、バリデーションや Entity 生成の責務をより明確に分割（Processor基盤への移行）。

### 5.2. PersonEditViewModel の競合状態対策 (リファレンス実装)
- [ ] **保存処理の排他制御 (Job管理)**:
    - [ ] `private var saveJob: Job? = null` を定義。
    - [ ] `save()` の開始時に `isActive` チェックによる二重実行防止ガードを実装。
- [ ] **状態更新とイベント発行の原子性検証**:
    - [ ] 保存成功時の `UiEvent.SaveSuccess` と `ViewEvent.NavigateBack` の発行順序とタイミングを精査。
- [ ] **初期ロードと編集操作の競合回避**:
    - [ ] `loadPerson` 完了前にユーザーが入力を開始した場合のデータ上書きを防止するガードの検討。
- [ ] **他 ViewModel への水平展開準備**:
    - [ ] 本対策を「リファレンス実装」として確立。
    - [ ] アプリ全体の破壊的操作（DB書き込み、ファイル操作、外部連携）を伴うメソッドをリストアップ。
        - [ ] CRUD系（各専門ViewModelのsave/delete）
        - [ ] メンテナンス系（バックアップ・復元・ログ削除）
        - [ ] 外部出力系（PDF生成）
- [ ] **二重保存の防止検証 (ユニットテスト)**:
    - [ ] 保存ボタンの連続タップを想定し、非同期処理実行中に次の保存要求が無視または適切に制御されることをテストで検証。
- [ ] **状態更新の原子性検証 (ユニットテスト)**:
    - [ ] 保存成功後の `UiEvent.SaveSuccess` 発行と `UiState` 更新が矛盾なく行われることを検証。

### 5.3. PersonListViewModel の検索整合性
- [ ] **検索結果の競合テスト**:
    - [ ] 前のキーワードの検索処理（遅い）が、後のキーワードの検索結果（速い）を上書きしないことを `flatMapLatest` の挙動を含めてテストで検証。
- [ ] **エッジケースの挙動確認**:
    - [ ] 検索クエリが空になった際、即座に全件表示に戻り、ローディング表示がハングしないことを検証。

## 🎯 8. テスト戦略の強化 ロードマップ

「4. テスト戦略の強化と再発防止対策」に基づく、実装および検証のステップです。

### 8.1. シナリオテストの構築
- [ ] サンプルデータ（バックアップ形式）のテスト環境への統合。
- [ ] 「一覧 → 詳細（A/B/C）への遷移とデータ整合性」シナリオの定義・実装。

### 8.2. UIテストの細粒度化
- [ ] `DateTimeInputFields` における 1文字ずつの入力検証テストの追加。

### 8.3. 診断機能の強化
- [ ] `PersonBaseUiStateViewModel` におけるガード発動時の AuditLog 記録実装。

## 🎯 記録項目の処理構造の最適化 ロードマップ

「記録項目の処理構造の最適化検討」に基づく、実装および検証のステップです。

### 7.1. 共通プロセッサ基盤の導入
- [x] **`HealthCategoryProcessor` インターフェースの定義**:
    - バリデーション、Entity 生成、重複チェック、および Repository 操作の抽象化。
- [x] **カテゴリ固有プロセッサの実装**:
    - 身長体重、バイタル、血糖値の各 Processor クラスの実装。
- [x] **プロセッサ・レジストリの作成**:
    - カテゴリ（Enum）から Processor を取得する仕組みの構築。

### 7.2. 既存ロジックのリファクタリング
- [x] **`BatchInputLogic` / `BatchInputViewModel` への適用**:
    - 大規模な `when` 分岐を Processor 呼び出しに置き換え、拡張性を向上させる。
- [x] **`PersonHealthLogic` / `PersonHealthViewModel` への適用**:
    - 詳細画面における保存・削除・履歴ロード処理の Processor 化。
- [x] **`HealthRepository` の一括保存処理の最適化**:
    - `insertHealthDataBatch` 内の型判定を Processor または共通インターフェース経由に整理。

### 7.3. 品質担保と検証
- [x] **プロセッサ単体のユニットテスト**:
    - 各 Processor が期待される Entity を生成し、正しくバリデーションを行うことを検証。
- [x] **統合テスト**:
    - Processor 経由での保存・例外ハンドリングが、従来の `when` 分岐による実装と等価であることを検証。
