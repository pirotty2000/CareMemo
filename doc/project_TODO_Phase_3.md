...
### 5.1. 【完了】BatchInputViewModel の堅牢化
- [x] **保存処理의 異常系テスト**:
    - [x] `insertHealthDataBatch` 実行中に例外をスローし、AuditLog に `ERROR` が正しく記録されることを検証。
    - [x] 保存失敗後も `UiState` の入力値（身長・体重等）がクリアされずに保持されていることを検証。
- [x] **ロジックの整理（将来の拡張性確保）**:
    - [x] カテゴリごとの `when` 分岐を整理し、バリデーションや Entity 生成の責務をより明確に分割（Processor基盤への移行）。

### 5.2. 【完了】PersonEditViewModel の競合状態対策 (リファレンス実装)
- [x] **保存処理の排他制御 (Job管理)**:
    - [x] `private var saveJob: Job? = null` を定義。
    - [x] `save()` の開始時に `isActive` チェックによる二重実行防止ガードを実装。
- [x] **状態更新とイベント発行の原子性検証**:
    - [x] 保存成功時の `UiEvent.SaveSuccess` と `ViewEvent.NavigateBack` の発行順序とタイミングを精査。
- [ ] **初期ロードと編集操作の競合回避**:
    - [ ] `loadPerson` 完了前にユーザーが入力を開始した場合のデータ上書きを防止するガードの検討。
- [x] **他 ViewModel への水平展開準備**:
    - [x] 本対策を「リファレンス実装」として確立。
    - [x] アプリ全体の破壊的操作（DB書き込み、ファイル操作、外部連携）を伴うメソッドをリストアップ。
        - [x] CRUD系（各専門ViewModelのsave/delete）
        - [x] メンテナンス系（バックアップ・復元・ログ削除）
        - [x] 外部出力系（PDF生成）
- [x] **二重保存の防止検証 (ユニットテスト)**:
    - [x] 保存ボタンの連続タップを想定し、非同期処理実行中に次の保存要求が無視または適切に制御されることをテストで検証。
- [x] **状態更新の原子性検証 (ユニットテスト)**:
    - [x] 保存成功後の `UiEvent.SaveSuccess` 発行と `UiState` 更新が矛盾なく行われることを検証。

### 5.3. 【完了】PersonListViewModel の検索整合性
- [x] **検索結果の競合テスト**:
    - [x] 前のキーワードの検索処理（遅い）が、後のキーワードの検索結果（速い）を上書きしないことを `flatMapLatest` の挙動を含めてテストで検証。
- [x] **エッジケースの挙動確認**:
    - [x] 検索クエリが空になった際、即座に全件表示に戻り、ローディング表示がハングしないことを検証。
