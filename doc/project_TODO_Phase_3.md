...
- [ ] **ロジックの整理（将来の拡張性確保）**:
    - [ ] カテゴリごとの `when` 分岐を整理し、バリデーションや Entity 生成の責務をより明確に分割。

### 4.2. PersonEditViewModel の競合状態対策
- [ ] **二重保存の防止検証**:
...
### 7.1. 共通プロセッサ基盤の導入
- [ ] **`HealthCategoryProcessor` インターフェースの定義**:
...
### 7.2. 既存ロジックのリファクタリング
- [ ] **`BatchInputLogic` / `BatchInputViewModel` への適用**:
    - 大規模な `when` 分岐を Processor 呼び出しに置き換え、拡張性を向上させる。
- [ ] **`PersonHealthLogic` / `PersonHealthViewModel` への適用**:
    - 詳細画面における保存・削除・履歴ロード処理の Processor 化。
- [ ] **`HealthRepository` の一括保存処理の最適化**:
