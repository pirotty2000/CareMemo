...
**目的:** ユーザーの入力やタップ操作が、適切な `SettingsUiAction` の発行と、それを受けた ViewModel 等のメソッド呼び出しに繋がることを検証する。

| ID     | テスト項目        | 操作                | 期待結果 (発行される Action / 挙動)                                              |
|:-------|:-------------|:------------------|:----------------------------------------------------------------------|
| ACT-01 | 伏せ字設定変更      | スイッチをタップ          | `MaskingChanged` アクションが発行され、ViewModel の更新メソッドが呼ばれること                 |
| ACT-02 | 開発者モード有効化    | バージョン情報を連続タップ     | `VersionClick` アクションが発行され、`handleVersionClick` が呼ばれること |
| ACT-03 | エクスポート開始     | 「バックアップ実行」をタップ    | `ExportClick` アクションが発行され、`exportLauncher` が起動すること                   |
| ACT-04 | 整合性チェック実行    | 「整合性チェック」をタップ     | `CheckIntegrity` アクションが発行され、`checkIntegrity()` が呼ばれること              |
| ACT-05 | 監査ログエクスポート開始 | 「監査ログをエクスポート」をタップ | `ExportLogsClick` アクションが発行され、`exportLogsLauncher` が起動すること           |
...
- `Settings_AuditLogExportButton`: 監査ログエクスポートボタン
