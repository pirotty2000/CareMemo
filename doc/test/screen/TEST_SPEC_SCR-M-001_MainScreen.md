...
|:-------|:----------|:------------------------------|:----------------------------------|
| DSP-01 | ローディング表示  | `screenState = Loading`       | 画面全体にローディングインジケータが表示されること         |
...
**目的:** 画面回転や画面遷移後の復帰時に、スクロール位置やフィルタ条件が維持されることを検証する。

| ID     | テスト項目      | 操作                       | 期待結果                                  |
|:-------|:-----------|:-------------------------|:--------------------------------------|
| RST-01 | リスト位置の復元   | リスト中ほどまでスクロールして画面回転      | 復帰後にスクロール位置（最上部に見えている項目）が維持されていること    |
| RST-02 | 検索条件の維持    | 検索クエリ入力後に詳細画面へ遷移し、戻る     | 戻った際に検索クエリが入力されたままであり、リストもフィルタされていること |
| RST-03 | セクション選択の維持 | セクション選択後にプロセス死(再現) -> 復帰 | 復帰時に選択していた五十音セクションが維持され、リストに反映されていること |
