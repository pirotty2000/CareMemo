# テスト仕様書 - OrphanedPhotoViewModel

- **対象テストコード:**
    - `app/src/test/java/jp/mydns/fujiwara/carememo/viewmodel/OrphanedPhotoViewModelTest.kt`

## 1. 概要
迷子写真管理画面（SCR-S-004）における UI 状態（UiState）の管理、不整合データのスキャン、写真の削除操作、および画面遷移イベントの正確性を検証する。
...
| ID | テスト項目 | 検証内容 | 期待結果 |
|:---|:---|:---|:---|
| INI-01 | 自動スキャン成功 | `OrphanedPhotoViewModel` のインスタンス化 | 内部で `loadOrphanedPhotos` が呼ばれ、スキャン結果が `orphanedPhotos` に反映されること |
