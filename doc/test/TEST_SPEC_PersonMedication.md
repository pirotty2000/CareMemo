...
| BH-03 | ダイアログの保存 | ダイアログで保存した際、ViewModelの `syncMedicationDay()` が呼ばれ、監査ログに `"SUCCESS"` が記録されること。 |
...
| ID | テスト項目 | 検証内容 |
| :--- | :--- | :--- |
| LG-01 | データ取得失敗時の安全性 | 月間データ取得中（Flow）に例外が発生した際、`isLoading` が `false` になり、監査ログに `"OTHER_ERROR"` 等が記録されること。 |
| LG-02 | 同期失敗時の安全性 | データの同期中に例外が発生した際、`isLoading` が `false` になり、監査ログに `"OTHER_ERROR"` 等が記録されること。 |
