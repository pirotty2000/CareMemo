...
| ID | テスト項目 | 検証内容 |
| :--- | :--- | :--- |
| BH-01 | データ保存の動作 | 数値を入力して登録ボタンを押した際、`onSaveRecord` が呼ばれ、監査ログに `"SUCCESS"` が記録されること。 |
...
| ID | テスト項目 | 検証内容 |
| :--- | :--- | :--- |
| LG-01 | データ取得失敗時の安全性 | 履歴データ取得中（Flow）中に例外が発生した際、`isLoading` が `false` になり、エラーダイアログが表示され、監査ログに `"OTHER_ERROR"` 等が記録されること。 |
| LG-02 | 保存失敗時の安全性 | データの保存中に例外が発生した際、`isLoading` が `false` になり、エラーダイアログが表示され、監査ログに `"OTHER_ERROR"` 等が記録されること。 |
