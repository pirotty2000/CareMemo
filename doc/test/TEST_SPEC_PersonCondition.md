...
| ID | テスト項目 | 検証内容 |
| :--- | :--- | :--- |
| LG-01 | データ取得失敗時の安全性 | 所見データ取得中（Flow）に例外が発生した際、`isLoading` が `false` になり、エラーダイアログが表示され、監査ログに "ERROR" が記録されること。 |
| LG-02 | 保存失敗時の安全性 | データの保存中に例外が発生した際、`isProcessing` が `false` になり、エラーダイアログが表示され、監査ログに "ERROR" が記録されること。 |
| LG-03 | 削除失敗時の安全性 | データの削除中に例外が発生した際、`isLoading` が `false` になり、エラーダイアログが表示され、監査ログに "ERROR" が記録されること。 |
| LG-04 | 写真保存失敗時の安全性 | 写真の保存中に例外が発生した際、`isProcessing` が `false` になり、エラーダイアログが表示され、監査ログに "ERROR" が記録されること。 |
| LG-05 | 写真削除失敗時の安全性 | 写真の削除中に例外が発生した際、`isProcessing` が `false` になり、エラーダイアログが表示され、監査ログに "ERROR" が記録されること。 |
