...
## 5. プロセス死復元テスト (State Restoration Test)

- **[MUST] 復元シナリオの検証**: 主要画面において、プロセス死相当の状態（`SavedStateHandle` の活用）から復帰した際に、以下の項目が正しく維持されていることを検証してください。
    - 未保存のユーザー入力値。
    - 編集開始時の比較基準 (baseline) に基づく `isChanged` の判定結果。
    - スクロール位置等の重要な UI 状態。
- **一過性状態の非復元確認**: Snackbar 表示や Job 実行状態などの Transient な状態が、復元後に不正に残っていないことを確認してください。
