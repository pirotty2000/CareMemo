...
| ID | 章・節 | 項目名 | 重要度 | 影響範囲 | 優先順位 | 検討結果・備考 |
|:---|:---|:---|:---|:---|:---:|:---|
| **A-1** | 2.1 | **プラットフォーム依存の排除 (Context)** | 高 | アプリ全体 (VM) | **A** | ViewModel のユニットテストを阻害する最大の要因。アーキテクチャの純粋化に必須。 |
| **A-2** | 1.1 | **AppMaintenanceRepository の整理** | 高 | メンテナンス機能 | **A** | ルール違反（Logicへの依存）が顕著であり、基盤の安定性に直結するため最優先。 |
| **A-3** | 4.1 | **起動時ロックの厳格化 (要件強制)** | 高 | アプリ起動時 | **A** | 業務利用における最低限のセキュリティ境界。初期状態を「ロック」から開始。 |
| **B-1** | 1.3 | **HealthCategoryProcessor の整理** | 高 | 健康記録機能全般 | **B** | 業務ロジックと副作用の分離は、テスト容易性の向上に大きく寄与する。 |
| **B-2** | 2.2 | **状態管理ルールの徹底 (UiState)** | 高 | 状態管理モデル | **B** | UI状態の「事実（Fact）」としての不変性を保つプロジェクト規約の遵守。 |
| **B-3** | 4.2 | **重要操作時の再認証 (Step-up Auth)** | 高 | PDF出力・データ管理 | **B** | アンロック状態でのデータ持ち出し（PDF化等）を阻止するための個別認証。 |
| **C-1** | 1.2 | **リポジトリ設計の標準化 (モデルケース)** | 中 | 部分（連絡先） | **C** | 特定機能に限定された依存解消であり、影響は局所的。 |
| **C-2** | 4.3 | **ロック仕様の単純化 (即時一本化)** | 高 | アプリ全体 (ロック) | **C** | 複雑なタイマー設定を廃止。利便性との兼ね合いを考慮し、中長期的な改善。 |
| **D-1** | 3.1 | **パッケージ構成の適正化** | 低 | 局所的 | **D** | コードの整理整頓（クリーンアップ）であり、機能的な緊急度は低い。 |
