...
| **B-1** | 1.3 | **HealthCategoryProcessor の整理**  | 高   | 健康記録機能全般    | **B** | 完了  | 業務ロジックと副作用の分離。テスト容易性の向上。                     |
...
| **C-1** | 1.2 | **リポジトリ設計の標準化 (モデルケース)**         | 中   | 部分（連絡先）     | **C** | 進行中 | 特定機能に限定された依存解消であり、影響は局所的。                    |
...
- [ ] **子テーブル・リポジトリへの横展開 (ADR #8 準拠)**:
    - [ ] **HealthRepository**: `save...` メソッドへの統合、`BatchInputViewModel` および `PersonHealthViewModel` での ID 事前確定。
    - [ ] **ConditionRepository**: `save...` メソッドへの統合、`PersonConditionViewModel` および `UnassignedPhotoViewModel` での ID 事前確定。
    - [ ] **MedicationRepository**: `save...` メソッドへの統合、`PersonMedicationViewModel` での ID 事前確定。
