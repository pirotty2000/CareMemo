 ...
 * バックアップ・リストア（エクスポート・インポート）時に使用する JSON 形式のデータ構造を定義します。
 * Room エンティティをそのまま JSON 化せず、外部保存に適した DTO (Data Transfer Object) として切り離すことで、
 * スキーマ変更時（論理削除フラグの有無等）の互換性を維持しやすくしています。
 *
 * 【導入の背景】
