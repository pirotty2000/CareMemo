 ...
 *
 * 【設計指針：レイヤー責務】
 * 1. データの正規化と保証：UI 状態 (`PersonEditUiState`) と永続化 Entity (`Person`) の境界において、
 *    空白のトリミングや和暦から西暦 (Instant) への変換、および厳密なバリデーションを行い、
 *    不正なデータが永続化層へ流入することを防ぎます。
 * 2. 変更検知の集約：初期状態と現在の入力内容の比較ロジックを一元管理し、ViewModel 側での「変更あり」判定
 *    および UI の保存ボタン活性制御の正確な根拠を提供します。
 */
