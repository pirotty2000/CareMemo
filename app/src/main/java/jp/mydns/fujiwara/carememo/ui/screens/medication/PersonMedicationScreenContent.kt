...
fun PersonMedicationScreenContent(
    ...
    when (uiState.screenState) {
        is PersonMedicationScreenState.Loading -> {
            Box(modifier = modifier.fillMaxSize().testTag("Medication_Loading")) {
                LoadingScreen()
            }
        }
        is PersonMedicationScreenState.Error -> {
            ErrorState(
        ...
        is PersonMedicationScreenState.Active -> {
            if (isExpanded) {
                // --- タブレット・横向き: 2カラムレイアウト ---
                ...
                Column(
                    ...
                    // コンテンツ
                    Box(modifier = Modifier.weight(1f)) {
                        if (uiState.operation is PersonMedicationOperation.Refreshing) {
