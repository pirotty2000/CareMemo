...
class PersonListViewModel(
        ...
        viewModelScope.launch {
            if (enabled) {
                ...
                if (canAuthenticate == BiometricManager.BIOMETRIC_SUCCESS) {
                    ...
                    sendUiEvent(UiEvent.ShowErrorDialog(
                        "設定できません",
    ...
    private val _personsWithMatchedConditions = _searchQuery
        .flatMapLatest { query ->
            if (query.isBlank()) kotlinx.coroutines.flow.flowOf(null)
            ...
            try {
                ...
                if (isZip) {
                    ...
                    if (jsonString != null) {
                        val backup = json.decodeFromString<CareMemoBackup>(jsonString)
                        repository.replaceAllData(backup)
                        sendUiEvent(UiEvent.ShowInfoDialog("復元完了", "データの復元が完了しました（写真は含まれません）。"))
                    }
                }
            } catch (e: Exception) {
