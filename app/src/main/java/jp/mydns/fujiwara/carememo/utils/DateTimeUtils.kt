 ...
 *
 * 【役割】
 * アプリ全体で使用される日付・時刻の操作、および日本語（和暦・曜日）を含む書式整形を一手に引き受けるユーティリティです。
 * ケア業務において重要な「和暦表示」と、データ整合性のための「タイムゾーン正規化」を両立します。
 *
 * 【主な機能】
 * ・和暦フォーマット：令和・平成・昭和に対応した、年月日および曜日の動的な文字列生成。
 * ・年齢計算：生年月日から現在（または特定日）時点の満年齢を正確に算出。
 * ・正規化：生年月日の保存時に「UTC 00:00:00」へ丸め込み、タイムゾーン差による日付のズレを防止。
 * ・比較・判定：誕生日が近い（近日誕生日）かどうかのロジック提供。
 * ・写真キャプション：撮影 Uri から日時を抽出し、一貫した形式（yyyy/MM/dd HH:mm）で生成。
 */
