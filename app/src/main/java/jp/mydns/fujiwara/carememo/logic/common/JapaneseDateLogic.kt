 ...
 *
 * 【主な機能】
 * ・西暦 (LocalDate) から和暦 (BirthEra) への変換。
 * ・和暦 (BirthEra, year, month, day) から西暦 (LocalDate) への変換。
 * ・改元日を考慮した厳密な日付バリデーション。
 *
 * 【設計指針】
 * 1. AppSpecifications で定義された改元日（START_DATE）と西暦オフセット（OFFSET_YEAR）を基準とする。
 * 2. 物理的な日付の存在（カレンダーの妥当性）と、歴史的な元号の範囲の整合性の両方をチェックする。
 * 3. アプリの仕様として 1900年以前（明治・大正など）は非サポートとする。
 */
