 ...
 * 1. 本クラスは Android API に依存しない Pure Kotlin ロジックであり、JUnit による高速な単体テストが可能です。
 * 2. 医学的・システム的な閾値は直接記述せず、必ず `AppSpecifications` を参照し、源泉を一元化します。
 * 3. 判定結果は UI での演出（警告色の表示等）の根拠となりますが、本レイヤーは判定（事実）のみを行い、
 *    具体的な演出方法（色の決定など）には関与しません。
 */
object HealthLogic {
