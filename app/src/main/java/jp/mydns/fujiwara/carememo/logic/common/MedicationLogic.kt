 ...
 *
 * 【役割】
 * 服薬管理（カテゴリE）に関するドメインロジックを提供します。
 *
 * 【設計指針：Pure Kotlin / Android 非依存】
 * 1. カレンダー生成ロジックは Android の UI コンポーネントに依存せず、`LocalDate` をベースとした純粋な日付グリッドを返します。
 * 2. 同期アクション (`SyncAction`) の判定により、UI 層の入力変化を最小限の DB 操作（Insert/Delete）に翻訳する責務を負います。
 * 3. 未来日付への記録制限など、データ整合性に関わる重要な業務ルールを本レイヤーで一元管理します。
 */
